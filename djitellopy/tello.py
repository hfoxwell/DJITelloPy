...
class Tello:
    ...
    def __init__(self, host=TELLO_IP, retry_count=RETRY_COUNT, vs_udp=VS_UDP_PORT):
        ...
        for field in state.split(";"):
            ...
            if key in Tello.state_field_converters:
                ...
                try:
                    ...
                    Tello.LOGGER.debug(
                        "Error parsing state value for {}: {} to {}".format(
                            key, value, num_type
                        )
                    )
                    Tello.LOGGER.error(e)
    ...
    def get_frame_read(
        self, with_queue=False, max_queue_len=32
    ) -> "BackgroundFrameRead":
    ...
    def send_command_with_return(
        self, command: str, timeout: int = RESPONSE_TIMEOUT
    ) -> str:
    ...
    def raise_result_error(self, command: str, response: str) -> bool:
        ...
        raise TelloException(
            "Command '{}' was unsuccessful for {} tries. Latest response:\t'{}'".format(
                command, tries, response
            )
        )
        ...
        if wait_for_state:
            ...
            for i in range(REPS):
                if self.get_current_state():
                    t = i / REPS  # in seconds
                    Tello.LOGGER.debug(
                        "'.connect()' received first state packet after {} seconds".format(
                            t
                        )
                    )
                    break
    ...
    def send_rc_control(
        ...
        if (
            time.time() - self.last_rc_control_timestamp
    ...
    def __init__(self, tello, address, with_queue=False, maxsize=32):
        ...
        try:
            ...
            self.container = av.open(
                self.address, timeout=(Tello.FRAME_GRAB_TIMEOUT, None)
            )
